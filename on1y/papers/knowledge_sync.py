...
PAPER_DISTILL_PROMPT_VERSION = "paper-library-v1"
...
def prepare_paper(storage: SqliteStorage, user_id: int, item: PaperItem) -> PaperItem:
    ...
    conn.commit()
    summary = (item.abstract or item.title).strip()
    if len(summary) > 800:
