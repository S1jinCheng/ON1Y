...
def load_subscription_settings() -> dict[str, Any]:
...
def save_subscription_settings(
    *,
    ...
    current = load_subscription_settings()
    ...
    logger.info("Saved subscription settings to %s", path)
...
def sync_since_date(platform: str) -> date | None:
    if platform not in PLATFORMS:
        raise ValueError(f"unsupported platform: {platform}")
    key = f"{platform}_sync_since"
    return _parse_iso_date(load_subscription_settings().get(key))
